# MNIST AutoEncoder Web-App Project
...
An interactive Deep Learning web application designed to demonstrate image compression, denoising, and reconstruction using a **convolutional autoencoder model trained on the MNIST handwritten digit dataset**.

Built around the model architecture specified in `AutoEncoders_DL.ipynb`, the application features a **React 18 frontend** and a **Flask backend API**. It includes custom canvas drawing inputs, real-time reconstructions, before/after comparison sliders, metric evaluations, and an interactive 2D latent space mapping viewer.
